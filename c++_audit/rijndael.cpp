...
void rijndael::encrypt(uint8_t* input, uint8_t* output)
{
	__m128i tmp1;
	int j;
...
void test_rijndael()
{
	__m128i key, input;
	uint8_t output[16]{};
	uint8_t tmp1[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	uint8_t tmp2[16] = { 0xca, 0xea, 0x65, 0xcd, 0xbb, 0x75, 0xe9, 0x16, 0x9e, 0xcd, 0x22, 0xeb, 0xe6, 0xe5, 0x46, 0x75 };
	input = _mm_loadu_si128((__m128i*)tmp1);
	key = _mm_loadu_si128((__m128i*)tmp2);
	rijndael cipher = rijndael(key);
	cipher.encrypt(tmp1, output);
